...
#Changed file path for loading data, update if needed!
...
female_updrs_faith_plot_ribbon <- ggplot(updrs_female_metadata, aes(x = updrstotal, y = Faith_PD)) +
  geom_point() +
...
male_updrs_faith_plot_ribbon <- ggplot(updrs_male_metadata, aes(x = updrstotal, y = Faith_PD)) +
  geom_point() +
...
## UPDRS Shannon's Diversity correlation (Sex Controlled) ##
...
# Remove missing metadata
updrs_female_shannon <- subset_samples(updrs_female_shannon, !is.na(updrstotal))
updrs_male_shannon   <- subset_samples(updrs_male_shannon, !is.na(updrstotal))
...
female_cor_updrs <- cor.test(~ Shannon + updrstotal,
...
male_cor_updrs <- cor.test(~ Shannon + updrstotal,
...
female_updrs_s_plot_ribbon <- ggplot(updrs_female_metadata, aes(x = updrstotal, y = Shannon)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, colour = '#8B1A00', fill = 'red', alpha=0.4) +
  theme_classic() +
  labs(x = "Female UPDRS Total",
...
#Make Shannon Correlation Plots with RIBBON - Males
#UPDRS Shannon Plot 'males' 
male_updrs_s_plot_ribbon <- ggplot(updrs_male_metadata, aes(x = updrstotal, y = Shannon)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, colour = 'blue4', fill = 'turquoise4', alpha=0.4) +
  theme_classic() +
  labs(x = "Male UPDRS Total",
...
###Shannon diversity correlation for MoCa
## UPDRS Shannon's Diversity correlation (Sex Controlled) ##
